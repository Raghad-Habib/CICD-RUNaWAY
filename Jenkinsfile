pipeline {
	...
	environment {
		...
		AWS_EB_APP_NAME = 'Raghad-Belt2-EB-app'
        AWS_EB_ENVIRONMENT_NAME = 'Raghadbelt2ebapp-env-1'
	...
	stages {
		...
		stage('Build') {
			...
			steps {
				sh 'docker build -t raghadhabib/runaway:latest .'
			}
		}
		...
		stage('Push') {
			...
			steps {
				sh 'docker push raghadhabib/runaway:latest'
			}
		}
